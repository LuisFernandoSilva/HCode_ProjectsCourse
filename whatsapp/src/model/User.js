...
export class User extends Model {
    ...
    //retorna o usuario logado pegando como referencia sua chave 
    save(){

        return User.getRef().doc(this.key).set(this.toJSON());

    }
    //adiciona o contato a uma coleçao ligada ao usuario logado
    addContact(contact){

        return User.getRef().doc(this.key).collection('contacts').doc(contact.email).set(contact.toJSON());

    }
    //lista os contatos e atualiza todas vez que e mudada
    getContacts(){
